...
public class Drawing extends Observable implements Originator<Figure>
{
	...
	public boolean addShapeFilter(ShapeFilter2 filter)
	{
		// TODO ... shapeFilters ...
		boolean add = shapeFilters.add(filter);
		if (add)
		{	
			update();
		}
	 ...
	 */
	// TODO décommenter lorsque prêt
	public boolean removeShapeFilter(ShapeFilter2 filter)
	{
		// TODO ... shapeFilters ...
		boolean rm = shapeFilters.remove(filter);
		if (rm)
		{		
			update();
		}
	...
	{
		// TODO ... lineFilters ...
		boolean add = lineFilters.add(filter);
		if (add)
		{	
			update();
		}
	...
	{
		// TODO ... lineFilters ...
		boolean rm= lineFilters.remove(filter);
		if (rm)
		{
			update();
		}
	...
	{
		Stream<Figure> fStream = figures.stream();
		if (filtering)
		{
			if (shapeFilters.size() > 0)
				fStream= fStream.filter(shapeFilters);
			if (lineFilters.size()>0 )
				fStream = fStream.filter(lineFilters);
			if (fillColorFilter != null)
				fStream = fStream.filter(fillColorFilter);
			if (edgeColorFilter != null)
				fStream = fStream.filter(edgeColorFilter);
